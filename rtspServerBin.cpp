...
static void media_prepared_cb (GstRTSPMedia * media)
{
  ...
  for (i = 0; i < n_streams; i++) {
    ...
    //g_signal_connect(rtpbin, "on-ssrc-active", (GCallback)on_ssrc_active, media);
...
static GstFlowReturn new_sample_handler(GstElement *sink, gpointer userData) 
{
    ...
    if (!buffer) {
        __LOG(LOG_CRIT, "[RTSP][%s:%d] ch%d buffer cannot get from sample", _FILE_, __LINE__, info->ch);
        gst_sample_unref(sample);
        return GST_FLOW_ERROR;
    }
...
void RtspServerBin::setBitrate(guint16 data)
{
    ...
    g_object_get(re.enc, "bitrate", &bps, NULL);
    __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d set bitrate : %d", _FILE_, __LINE__, rtspServerData.ch, bps);
...
void RtspServerBin::setFps(guint16 data)
{
    ...
    //g_object_get(re.capsfilter, "caps", caps, NULL);
    __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d set fps : %s", _FILE_, __LINE__, rtspServerData.ch, gst_caps_to_string(caps));
...
void RtspServerBin::setRotation(guint16 data)
{
    ...
    __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d set rotation : %d", _FILE_, __LINE__, rtspServerData.ch, rotation);
...
void RtspServerBin::setGop(guint16 data)
{
    ...
    __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d set gop_size : %d", _FILE_, __LINE__, rtspServerData.ch, gop);
...
void RtspServerBin::setkeyframe(guint16 data)
{
    ...
    g_object_get(re.enc, "set-keyframe", &key, NULL);
    __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d set keyframe : %d", _FILE_, __LINE__, rtspServerData.ch, key);
...
RtspServerBin::~RtspServerBin()
{
    __LOG(LOG_INFO, "[GST][%s:%d] %s[%d]", _FILE_, __LINE__, __FUNCTION__, rtspServerData.ch);
}

gboolean RtspServerBin::addBinToPipe(GstElement *pipe)
{
    if(gst_bin_get_by_name(GST_BIN(pipe), g_strdup_printf("captureBin%d", rtspServerData.ch)) != NULL)
    {
        __LOG(LOG_NOTICE, "[GST][%s:%d] ch%d capture bin is already added", _FILE_, __LINE__, rtspServerData.ch);
        return 1;
    }
...
gboolean RtspServerBin::addBinTeeRecordPad(guint8 ch)
{
    ...
    teeRecordPad = gst_ghost_pad_new(g_strdup_printf("record_pad_ch%d", ch), gst_element_get_request_pad(re.tee, "src_%u"));
...
GstPad* RtspServerBin::getBinTeeRecordPad()
{
    if(teeRecordPad == NULL)
        __LOG(LOG_ERR, "[GST][%s:%d] %s ch:%d pad is null", _FILE_, __LINE__, __FUNCTION__, rtspServerData.ch);
    else
        __LOG(LOG_INFO, "[GST][%s:%d] %s ch:%d", _FILE_, __LINE__, __FUNCTION__, rtspServerData.ch);

    return teeRecordPad;
}

gboolean RtspServerBin::getDualBps()
{
    return rtspServerData.dual_bps;
}

void RtspServerBin::setDualBps(gboolean val)
{
    //__LOG(LOG_NOTICE, "[GST][%s:%d] %s ch:%d val:%d", _FILE_, __LINE__, __FUNCTION__, rtspServerData.ch, val);
...
gboolean RtspServerBin::init(guint8 ch, gboolean crop_en)
{
    ...
    if (!re.bin || !re.queue || !re.capsfilter || !re.parse || !re.enc || !re.rate || !re.sink || !re.convert || !re.queue2 || !re.crop \
        || !re.overlay || !re.compositor || !re.convert2 || !re.capsfilter2 || !re.videoflip || !re.identity || !re.tee) {
        __LOG(LOG_CRIT, "[GST][%s:%d] rtsp element create error", _FILE_, __LINE__);
        return ret;
    }
    ...
    sinkPad = gst_ghost_pad_new(g_strdup_printf("rtspServerBin_sink_ch%d", rtspServerData.ch), gst_element_get_static_pad(re.queue, "sink"));
    //sinkPad = gst_ghost_pad_new(g_strdup_printf("rtspServerBin_sink_ch%d", rtspServerData.ch), gst_element_get_static_pad(re.sink, "sink"));
    ret = gst_element_add_pad(re.bin, sinkPad);
    if(!ret) {
        __LOG(LOG_CRIT, "[GST][%s:%d] rtsp pad add err", _FILE_, __LINE__);
        return ret;
...
void rtspServerStop()
{
    ...
    if(cleanSesson_id) g_source_remove(cleanSesson_id);
    if(removeSesson_id) g_source_remove(removeSesson_id);
}
...
guint rtspServerStart()
{
    ...
    //GstRTSPSessionPool *session_pool = gst_rtsp_server_get_session_pool(rtspServer);
    //g_object_set(session_pool, "max-sessions", 10, NULL);
    //g_object_set(session_pool, "timeout", 60, NULL);
    //gst_object_unref(session_pool);
    
    //cleanSesson_id = g_timeout_add_seconds (DEFAULT_RTSP_SESSION_CLEAN_PERIOD, (GSourceFunc) cleanRtspSession, rtspServer);
    cleanSesson_id = g_timeout_add_seconds (cmdArg.duration*60, (GSourceFunc) cleanRtspSession, rtspServer);
    //removeSesson_id = g_timeout_add_seconds (60, (GSourceFunc) remove_sessions, rtspServer);
