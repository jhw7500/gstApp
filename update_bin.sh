#!/bin/bash
# bin/gstApp 을 pim-package 배포 트리에 복사하고 매니페스트를 맞춘다.
#
...
ARTIFACT="$HERE/bin/gstApp"
DEST_REL="dist/pim/usr/local/bin/gstApp"
BUILD_HINT="./make-for-imx8 -j4"
...
usage() {
    cat <<'USAGE'
update_bin.sh — bin/gstApp 을 pim-package 배포 트리에 복사하고 매니페스트를 맞춘다.
