...
int main(int argc, char *argv[]) {
    ...
    thread = g_thread_new("pipeline thread run", record_thread_run, NULL);

    // RTSP 서버 생성
    GstRTSPServer *server = gst_rtsp_server_new();
    GstRTSPMountPoints *mounts = gst_rtsp_server_get_mount_points(server);

    // GStreamer RTSP 미디어 팩토리 생성
    GstRTSPMediaFactory *factory = gst_rtsp_media_factory_new();

    // /dev/video3 장치로부터 영상 데이터를 받아서 RTSP 서버에 연결하는 파이프라인 설정
    gchar *launch_str = g_strdup_printf("( v4l2src device=/dev/video3 ! imxvideoconvert_g2d ! vpuenc_h264 ! h264parse ! rtph264pay name=pay0 pt=96 )");
