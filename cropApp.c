...
#else
    if(!gst_element_link_many(pipe->queue, pipe->videorate, pipe->encoder, pipe->sink, NULL))
...
#else
    if (!gst_element_link_many(pipe->queue, pipe->videorate, pipe->encoder, pipe->parse, NULL))
#endif
    {
...
gint main(int argc, char *argv[]) 
{
    ...
    // 파이프라인 생성
    guint start_index = 0;
    guint end_index = MAX_SRC;
    guint ch0_en = 1, ch1_en = 0, ch2_en = 1, ch3_en = 1;
    //pipeline = gst_pipeline_new("pipeline");
    //i = 1;

    guint8 ch_enable = 0x0f;    //(ch3_en&0x01)<<3 + (ch2_en&0x01)<<2 + (ch1_en&0x01)<<1 + (ch0_en&0x01);
    g_message("channel enable bit %02x", ch_enable);
    main[0].crop_enable = FALSE;
    main[1].crop_enable = FALSE;
    if((ch_enable&0x03) == 0x03)
    {
        g_print("crop0\n");
        main[0].crop_enable = TRUE;
    }
    if((ch_enable&0x0c) == 0x0c)
    {
        g_print("crop1\n");
        main[1].crop_enable= TRUE;
    }
    ...
    {
        //guint8 queue_leaky = LEAKY_DOWNSTREAM;
        if(((ch_enable>>(i*2))&0x03) == 0x03)
        {
            g_message("main[%d].crop true", i);
            main[i].crop_enable = TRUE;
        }
        ...
        if(main[i].crop_enable)
        {
            g_message("main[%d] crop_enable", i);
            caps = gst_caps_new_simple("video/x-raw",
                                                "format", G_TYPE_STRING, "NV12",
                                                "width", G_TYPE_INT, _WIDTH*2,
                                                "height", G_TYPE_INT, _HEIGHT,
            ...
            main[i].crop[0] = gst_element_factory_make("videocrop", g_strdup_printf("crop%d_L", i));
            main[i].crop[1] = gst_element_factory_make("videocrop", g_strdup_printf("crop%d_R", i));
            g_object_set(main[i].crop[0], "top", 0, "bottom", 0, "left", _WIDTH, "right", 0, NULL);
            g_object_set(main[i].crop[1], "top", 0, "bottom", 0, "left", 0, "right", _WIDTH, NULL);

            gst_bin_add_many(GST_BIN(main[i].pipeline), main[i].crop[0], main[i].crop[1], NULL);
            gst_element_link(main[i].tee, main[i].crop[0]);
            gst_element_link(main[i].tee, main[i].crop[1]);

            main[i].tee2[0] = gst_element_factory_make("tee", g_strdup_printf("tee%d_L", i));
            main[i].tee2[1] = gst_element_factory_make("tee", g_strdup_printf("tee%d_R", i));
            gst_bin_add_many(GST_BIN(main[i].pipeline), main[i].tee2[0], main[i].tee2[1], NULL);
            gst_element_link(main[i].crop[0], main[i].tee2[0]);
            gst_element_link(main[i].crop[1], main[i].tee2[1]);

            //main[i].pad[0] = gst_element_get_static_pad(main[i].crop[0], "src");
            //main[i].pad[1] = gst_element_get_static_pad(main[i].crop[1], "src");
        }
        else
        {
            g_message("main[%d] crop_disable", i);
            caps = gst_caps_new_simple("video/x-raw",
                                                "format", G_TYPE_STRING, "NV12",
                                                "width", G_TYPE_INT, _WIDTH,
                                                "height", G_TYPE_INT, _HEIGHT,
            ...
            main[i].tee2[0] = main[i].tee;
            main[i].tee2[1] = main[i].tee;
        }
    }
    ...
    {
        if((ch_enable&(0x01<<i)) == 0) 
        {
            g_message("channel %d disable", i);
            continue;
        }
        g_message("channel %d enable", i);
        ...
        if (!gst_element_link(main[src_num].tee2[i%2], main[src_num].videoPipe[dir].queue))
        {
