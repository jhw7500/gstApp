...
#### 스냅백-롤오버 충돌은 운영 임계에서 열리지 않는다

자연 롤오버는 `duration*60` 이 분의 정수배이므로 **항상 조각이 열린 분내 초에 재발**한다.
강제 분할은 `startSec` 직후 0~0.5초에 난다. 따라서 두 사건의 간격은 대략 drift 이고,
강제 분할 조건은 `drift >= split_max_msec` 이다. **임계값 자체가 가드**인 셈이다.
...
- 커널 모듈 `max9296.ko` 가 다른 세션의 수정본으로 교체된 상태였다
  (srcversion `9E94D01E901C66E7416A46A`). 세 갈래를 같은 드라이버 아래 연속 실행했으므로
  대조의 내부 타당성에는 영향이 없다.
...
2026-09-04 런은 `tmp/target-192.168.214.4/record_skew_4ch_20260904/`.
...
47. 스냅백 강제 분할이 자연 롤오버와 겹치는 경로는 운영 기본 임계(`split_max_msec=5000`)
    에서 구조적으로 열리지 않는다. 자연 롤오버가 조각이 열린 분내 초에 재발하므로 두
    사건의 간격이 대략 drift 이고, 강제 분할 조건이 `drift >= 임계` 라 간격이 항상 GOP
    보다 크다. `-X 100` 으로 임계를 낮출 때만 충돌이 열린다 (§3.32).
