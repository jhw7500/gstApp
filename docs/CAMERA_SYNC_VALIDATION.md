...
### 3.31 4채널 분할 경계 절대 skew 통제 측정 (1080p15 / 720p30 / 360p60)

#### 목적

이슈 #55가 지목한 공백은 "측정 구간 시작 시점에 이미 존재했을 수 있는 상수
오프셋"이다. `test/run-record-sync-check.sh`(#54)의 파일별 프레임 수 비교는 구간
안에서 **오차가 변했는지**만 보므로 이 값을 원리적으로 볼 수 없다. 경계가
`[0,900,1800]`인 채널과 `[1,901,1801]`인 채널이 둘 다 900프레임 파일을 만든다.

이 절은 그 상수 오프셋을 `splitCheck()`의 running-time 기준 skew로 직접 측정한
기록이다. 이 값은 `splitmuxsink-fragment-opened`가 전달하는 조각 첫 내용의
running-time 차이이고(`muxSinkBin.cpp:181` 저장, `main.cpp:563` 비교),
running-time은 단일 최상위 pipeline의 공통 시간축이므로 채널 간 차이가 곧 절대
오프셋이다. 파일 프레임 수 비교와 달리 이 값은 상수 오프셋을 그대로 드러낸다.
...
| 측정일 | 2026-09-04 |
| 타겟 | `pim-camera-v016` (192.168.214.4) |
| 바이너리 | 운영 `/usr/local/bin/gstApp` 복사본, md5 `7c1afbd26fd2be4e51b10c0c9aef1ac5` (2,118,624 B, 2026-09-03) |
| 절차 | `/root/skew55a/run-skew55a.sh <W> <H> <FPS> 15` |
| 조건당 측정 시간 | 15분, 분할 주기 `duration=2`(2분) |

이 시험을 위한 별도 빌드는 하지 않았다. 운영 바이너리에 PR #53의 running-time
skew 판정이 이미 들어 있어(`strings`로 `skew basis`·`split_running_time` 확인)
같은 바이너리를 CLI 인자만 바꿔 실행했다.

통제 조건은 세 시험 공통이며, 모두 실행 로그로 확인했다.

| 항목 | 값 | 확인 로그 |
| --- | --- | --- |
| 채널 | 4채널 enable | `chEn:0xf` |
| ISP | 컴파일 기본값으로 4채널 동일 — `ae_on=1 gain=256 exp_time=10000 awb=auto hflip=0 vflip=0` | `videoBin.cpp:850/877` 네 줄이 완전 동일 |
| LED flash | 4채널 off | `led_flash: enable=0 ... flash_reg=0x0000` ×4 |
| 로그 | `debug_level=7` | `dbgLevel:7` |

배포 config에서 바꾸지 않은 것은 채널별 bitrate(8192/4096/2048/1024 kbps),
`gop=[30,15]`, `enc=h265`, `duration=2`, `tmp_path=/dev/shm`이다. 비트레이트를
균일화하지 않은 이유는 §3.7·§3.8이 배포 비트레이트 상태에서 이미 네 채널 인코더
출력 PTS 동일을 실증했고, 균일화하면 결과가 배포 구성을 대표하지 못하기 때문이다.

> **로그 경로 주의.** NOTICE 이상은 rsyslog가 `local0.notice` 필터로
> `/var/log/cantops/local0.log`에 남기므로 **DEBUG는 이 파일에 오지 않는다.**
> 분할마다 찍히는 skew 로그(`main.cpp:585`, `LOG_DEBUG`)는 `dbg_level` stdout
> 갈래(`util.cpp:312`)를 `-g 7`로 열어 파일로 직접 redirect해 수집했다. NOTICE
> 쪽(`main.cpp:578`)은 `reported_basis` 가드 때문에 기준·채널수가 바뀔 때만
> 찍히므로 분할별 표본이 되지 못한다.

#### 절차상 필수 발견 — 하드웨어 epoch 을 올리지 않으면 구성을 바꿀 수 없다

첫 시도는 gstApp 기동 실패로 끝났다. 원인은 fps 상한이 아니었다.

```
[crit][MAX9296_PREPARE] CSI0 tuple=1280x360@60 enable=3 primary_errno=-116
                            before_state=4 after_state=4
[crit][MAX9296_PREPARE] CSI1 tuple=1280x360@60 enable=3 primary_errno=-116
                            before_state=0 after_state=0
```

`-116`은 `-ESTALE`이다. max9296 드라이버는 한 하드웨어 epoch 안에서 prepare
fingerprint(해상도·fps·enable 조합) 변경을 거부한다 — `max9296.c:4795-4801`과
`:5079-5087`의 판정이며, 주석은 "Changing exact dual/left/right programming
within one hardware epoch is unsafe because serializer address routing may
already have changed"로 사유를 밝힌다.

epoch은 실제 전원 전환(`max9296.c:2134`) 또는 마지막 power user 해제(`:6701`)에서만
오른다. `systemctl stop cam-operate` 뒤 프로세스를 종료하는 것만으로는 오르지
않는다 — 시험 전후 sysfs `prepare`가 모두 `epoch=2`로 이를 직접 보여준다.
CSI1이 `before_state=0`(깨끗한 유휴)인데도 같이 실패한 것이, 잔존 상태 문제가
아니라 구조적 거부임을 가른 단서였다.

따라서 절차에 `/opt/pim/bin/cam_hard_reset.sh`를 **시험 전과 복원 시 양쪽 모두**
넣었다. 복원 시가 특히 중요하다. 시험이 성공해 하드웨어가 새 지문으로
프로그래밍되면 **운영 구성으로의 재기동도 ESTALE로 막히기 때문**이다. 첫 시도가
prepare 단계에서 죽어 지문이 바뀌지 않았던 덕에 그때는 운영 복귀가 우연히
성공했을 뿐이다.

#### 결과

| 조건 | prepare tuple | skew 표본 | skew | wall-skew (min/중앙/max) | ISI 실측 / 기대 |
| --- | --- | --- | --- | --- | --- |
| 1920×1080@15 | `3840x1080@15 enable=3` | 10 | 전량 0ms | 12 / 13 / 19 ms | 29.96 / 30 /s |
| 1280×720@30 | `2560x720@30 enable=3` | 10 | 전량 0ms | 6 / 7 / 19 ms | 59.20 / 60 /s |
| 640×360@60 (1차) | `1280x360@60 enable=3` | 11 | 전량 0ms | 1 / 3 / 16 ms | 122.70 / 120 /s |
| 640×360@60 (2차) | `1280x360@60 enable=3` | 11 | **10건 0ms + 1건 16ms** | 0 / 4 / 33 ms | 118.60 / 120 /s |

전 42표본의 `active`는 4였고 판정 기준은 모두 `running-time`이었다. `wall-clock`
으로 떨어진 회차는 없다. 0이 아닌 표본은 **42건 중 1건**이며, 아래에 별도로
분석한다.

**실측 프레임률.** ISI 인터럽트 증가량으로 확인했다. ISI는 CSI당 결합 프레임
하나를 세고 그 안에 2채널이 들어가므로(§2.1) 기대값은 `fps × 채널수`가 아니라
`fps × CSI수(2)`다. 세 조건 모두 기대값과 일치했다. 특히 360p@60은 자동 노출이
노출시간을 프레임 주기(16.7ms) 이상으로 밀어 실제 캡처가 60fps에 미달할
가능성이 있었는데, 122.70/s는 30fps일 때의 60/s가 아니라 60fps일 때의 120/s와
맞으므로 그 우려는 실현되지 않았다.

**노출 경로는 조건마다 다르다.** `max9296_exposure_plan()`
(`max9296Controls.cpp:131-141`)은 `fps <= 30`이면 `WRITE_EXPOSURE_SEED`를
반환해 `exp_time`을 하드웨어에 기록하지만, 60fps에서는 네 채널이 모두 auto AE라
`SKIP_EXPOSURE_SEED`가 되어 **`exp_time` 값이 적용되지 않는다**
(로그: `[MAX9296_EXPOSURE] skip seed csi=0 ... fps=60 policy=auto-ae-high-fps`).
같은 이유로 `over_period` 지표는 경고 분기 안에서만 계산되므로 이 설정에서는
아예 출력되지 않는다. 60fps의 실제 프레임률 판정에 ISI 카운터를 쓴 이유가 이것이다.

**0이 아닌 유일한 표본 — 스냅백 직후 1프레임 과도현상.** 640×360@60 2차
실행의 10:03:00 평가에서 `skew:16ms`가 나왔다. 16ms는 60fps의 정확히 한
프레임(16.67ms)이다. 원인은 로그에 직접 남아 있다.

기동 직후 첫 정렬이 임계를 넘어(`smMax:5480ms` > `split_max_msec` 5000ms)
10:01:00에 강제 분할이 걸렸고, 그때 열린 조각의 running-time이 채널마다
갈렸다.

```
10:01:00:440 [err]  Snap-back split (diff:0s, smMax:5480ms, skew:0ms/running-time,
                    wall-skew:33ms)
10:01:00:441        ch0/ch1/ch2/ch3 Force keyframe event sent   (네 채널 모두)
10:01:00:889        ch2 Fragment opened ... time: 0:00:55.083
10:01:00:896        ch1 Fragment opened ... time: 0:00:55.066   <- 1프레임 이름
10:01:00:896        ch0 Fragment opened ... time: 0:00:55.083
10:01:00:897        ch3 Fragment opened ... time: 0:00:55.083
```

강제 키프레임은 네 채널 모두에 전송됐다. 그럼에도 ch1만 한 프레임 이른 지점에서
잘렸고, 그 경계가 다음 평가(10:03:00)에서 16ms로 관측됐다. **다음 정규 분할부터
회복돼 10:05~10:17의 여덟 회 연속 분할이 모두 0ms였다.**

같은 조건 1차 실행에도 스냅백이 있었지만(07:43:00, `smMax:5210ms`) 그때는 네
채널이 모두 `0:00:55.066`으로 일치했다. 즉 **스냅백이 항상 어긋남을 만드는 것이
아니라 만들 수 있다** — 관측 2회 중 1회다. 1920×1080@15와 1280×720@30 실행에서는
스냅백이 한 번도 발생하지 않았으므로(전 조각의 4채널 running-time이 완전 일치),
이 경로가 낮은 프레임률에서도 같은 결과를 내는지는 이 시험으로 알 수 없다.

이 값은 이슈 #55가 물은 **상수 오프셋이 아니다.** 한 번 나타났다 다음 분할에서
사라졌고, 정규 분할 구간에서는 네 조건 모두 0이었다. 다만 강제 정렬 경로가 한
프레임 어긋남을 만들 수 있다는 것은 이 시험에서 처음 관측된 사실이다.

**파일 프레임 수 대조.** 감시자(`cam-operate`)를 정지한 상태에서는 완료 조각이
`/dev/shm/recordings`로 옮겨지지 않고 `/dev/shm`의 닫힌 `.part`로 남는다. 측정
종료 시 앱을 SIGTERM으로 정상 종료해 마지막 조각을 마감한 뒤 복원 전에 파일을
확보하고 `test/run-record-sync-check.sh`로 대조했다.

| 조건 | 완전한 분 | ch0 | ch1 | ch2 | ch3 | 최대 차이 | 키프레임(4채널) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1920×1080@15 | 2개 | 1800 | 1800 | 1800 | 1800 | 0 프레임 (0.0 ms) | 60 동일 |
| 1280×720@30 | 2개 | 3600 | 3600 | 3600 | 3600 | 0 프레임 (0.0 ms) | 120 동일 |
| 640×360@60 (2차) | 2개 | 7200 | 7200 | 7200 | 7200 | 0 프레임 (0.0 ms) | 240 동일 |

프레임 수는 `fps × 120초`(`duration=2`)와 정확히 일치한다. 누적(완전한 분 4개)도
네 채널이 각각 7200 / 14400 / 28800으로 동일했다. 다만 이 대조가 덮는 구간은
측정 후반부이며, 위 스냅백 과도현상이 있었던 10:01~10:03 조각은 수집 대상에
포함되지 않았다 — 그 구간의 어긋남은 running-time 로그로만 관측했다.

#### 판정과 한계

정규 분할 구간에서 네 채널의 분할 경계는 **절대 기준으로 1ms 미만 일치**했다
(42표본 중 41건 0ms). 프레임 주기가 66.7ms(15fps)에서 16.7ms(60fps)로 4배
좁아져도 이 결과는 바뀌지 않았다. 따라서 이슈 #55가 남겨 둔 "구간 시작 시점의
상수 오프셋"은 **적어도 1ms 해상도에서 존재하지 않는다.**

예외는 스냅백 강제 분할 직후의 1건(16ms = 60fps의 1프레임)이며, 상수가 아니라
다음 분할에서 사라지는 과도현상이다. 이 경로는 별도 항목으로 남긴다.

한계는 다음과 같다.

1. 이 측정이 증명하는 것은 **인코더 출력 이후 분할 경계의 일치**다. 센서 노출
   위상 자체는 여전히 미증명이며(§2.1, §5.6), 타임스탬프 기반 계측은 원리적으로
   이를 볼 수 없다 — 네 센서가 다른 순간을 노출해도 이후 체인의 PTS는 동일하게
   찍히기 때문이다. 이 항목은 광학 자극이 필요하다.
2. 이슈 #55 본문이 처방한 밀리초 스톱워치 동시 촬영은 **해상도가 1프레임**
   (15fps 기준 66.7ms)이므로, 여기서 쓴 running-time skew(1ms)보다 66배 거칠다.
   스톱워치는 위 1번(센서 노출 위상)을 위한 수단이지 분할 경계 확정 수단이 아니다.
3. 조건당 15분·표본 10~11건이다. 장시간 운용에서의 드리프트는 다루지 않았다.
4. 표본은 분할 시점에만 생성되므로 분할 사이 구간은 관측하지 않는다.
5. 스냅백 강제 분할 경로는 640×360@60에서만 2회 관측됐고(그중 1회에서 1프레임
   어긋남), 1920×1080@15·1280×720@30에서는 발생하지 않았다. 낮은 프레임률에서
   같은 경로가 같은 결과를 내는지는 미측정이다. 스냅백을 의도적으로 유발해
   반복 관측하는 시험이 필요하다.

#### 이력 로그와의 대조

같은 계측기가 남긴 과거 출력도 함께 전수 확인했다. `/var/log/cantops/local0.log`
전 로테이션(2026-03-16~09-03)에서 `skew basis` 195건을 추출해 각 표본을 그 시점의
구성과 조인한 결과, **running-time 기준 193건 전부 `skew:0ms`**였다. 0이 아닌
2건은 모두 running-time을 얻지 못해 `wall-clock`으로 되돌아간 회차였다(44ms /
2ms — 벽시계 잡음이지 내용 오차가 아니다). 4채널 표본은 1920×1080@15가 4건,
1280×720@30이 11건이었고 전부 0ms였다. 다만 이 이력 표본들은 LED가 ch0·ch2에서
켜져 있었고 ISP 설정도 채널마다 달랐으므로, 통제 조건 측정은 이 절이 처음이다.

부수적으로 `wall-skew`가 채널 수에 따라 커지는 것이 관측된다 — 2채널 0~17ms,
4채널 23~58ms(이력, FHD/HD). 임계 `split_diff_msec`이 200ms이므로 최대 108ms는
그 절반을 넘는다. running-time은 같은 순간 전부 0이므로, PR #53의 판정축 전환이
확보한 여유가 정량적으로 확인된 셈이다.

#### 원시 자료

- 조건별 실행 로그와 runner 로그: `tmp/target-192.168.214.4/record_skew_4ch_20260904/`
- 절차 스크립트: 타겟 `/root/skew55a/run-skew55a.sh`

각 조건 종료 후 원복은 config md5 대조(`ce7517af84b80aeef65710fd5fab7b77`),
`cam_hard_reset.sh` 재실행, `cam-operate` 재기동과 운영 prepare 상태
(`state=CONSUMED width=1280 height=360 fps=30 errno=0`) 확인으로 검증했다.

### 3.32 이슈 #94 — 같은 분 이중 분할과 파일 덮어쓰기 (원인·되돌림 대조)

#### 목적

같은 분에 분할이 두 번 일어나면 두 조각이 같은 파일명을 쓴다(`format_location` 이
`%Y%m%d_%H%M00` 로 분 단위 반올림). 덮어쓰기가 실제로 일어나는지, 원인이 무엇인지,
수정이 그 원인을 제거하는지를 확인한다.

#### 덮어쓰기 메커니즘 (타겟, 카메라 불필요)

`filesink` 는 기존 파일을 **같은 inode 에 in-place truncate** 한다.

```
dd if=/dev/urandom of=/tmp/probe.bin bs=1024 count=1024
before: inode=94878 size=1048576
gst-launch-1.0 fakesrc num-buffers=1 sizetype=2 sizemax=10 ! filesink location=/tmp/probe.bin
after : inode=94878 size=10
```

`splitmuxsink` 의 `location` 에서 포맷 지정자를 빼면 매 조각이 같은 경로를 연다. 이것이
`format_location` 이 같은 분에 같은 이름을 두 번 돌려주는 상황과 동일하다. `location`
한 글자만 다른 대조군을 같이 돌렸다(`videotestsrc num-buffers=150` -> `jpegenc` ->
`avimux`, `max-size-time=1000000000`, `async-finalize=false`).

| 런 | location | 결과 파일 | 크기 |
| --- | --- | --- | --- |
| 대조군 | `probe_seq_%05d.avi` | 5개 | 합 1,856,754 B |
| 실험군 | `probe_same.avi` | **1개** | **371,218 B** |

5조각이 만들어졌는데 살아남은 것은 한 조각 분량뿐이다. 손실은 `/dev/shm` 단계이며
이관 스크립트(`chk_cam_operate.sh`)가 개입하기 전이다 — 그 스크립트는
`session_YYYYmmdd_HHMM.all_done` 마커로 돌면서 현재 분은 미루므로 0.5초 안에 낄 수 없다.

#### 원인 — `need_first_split` 이 강제 경로에서 소진되지 않는다

`splitCheck()` 의 `need_first_split` 은 **정렬됨 경로에서만** 지워졌다. 첫 정렬 판정이
강제(스냅백) 경로로 가면 플래그가 살아남아, 이후 첫 '정렬됨' 틱에서 분할할 이유가
없는데도 분할을 한 번 더 낸다. `splitmuxsink` 의 분할 시그널은 다음 GOP 에 지연
발화하므로(`split-at-running-time` 은 `times_to_split` 큐에 적재되고 자연 롤오버로
비워지지 않는다), 그 분할이 자연 `max-size-time` 롤오버와 겹치면 조각 하나가 1 GOP
만에 잘린다.

2026-09-04 자연 조건 런(640x360@60, 4채널, `recording_time=2`)의 원 관측:

```
07:42:05:605  next split time : 43m 00s        need_first_split=TRUE (시작 초 05 != 00)
07:43:00:183  Snap-back split (smMax:5210ms)   강제 경로 - 플래그 미소진
07:45:00:359  All channels aligned             남은 플래그가 여기서 발화 (당시 무로그)
07:45:00:633  dur=1:59.933                     자연 롤오버
07:45:01:133  dur=0:00.500                     지연 발화분 - 같은 파일명, 앞 조각 덮어씀
07:47:00:022  All channels aligned  -> 단일 분할 (플래그 소진 후 재발 없음)
```

그 시점 `skew:0ms`, `drift:626ms` 로 분할할 이유가 전혀 없었다. 07:47 이후 재발이
없는 것이 대조군이다.

#### 되돌림 대조 (2026-09-08, 640x360, 2채널, `recording_time=1`)

master 에 **로그 2줄만** 넣고 수정은 넣지 않은 `logonly` 빌드를 따로 만들어, 되돌린
동작에서 결함이 실제로 재현되는 것을 먼저 확인했다. 동일 조건: 초 05 기동 -> 첫 조각
`split_msec` 약 10.3s >= 임계 5000 -> 첫 판정이 강제 경로. 앱 인자 `-d 5 -m 4 -g 7`.

| 빌드 | 강제 진입 `smMax` | `initial alignment split` | 같은 분 이중 절단 | 5초 미만 조각 | 파일명 중복 |
| --- | --- | --- | --- | --- | --- |
| `logonly` (master 동작) | 10309ms | **1건** | **발생** | **2건** | **있음** |
| `fix` | 10360ms | **0건** | 없음 | **0건** | 없음 |

```
logonly:
  03:39:01:308  조각#2  rt=0:00:50.300                      자연 임계 = 110.300
  03:40:00:427  initial alignment split at rt 110365547620  누수된 플래그 발화 (T=110.3655)
  03:40:00:683  조각#3  rt=0:01:50.000  dur=0:00:59.700     자연 롤오버
  03:40:01:321  조각#4  rt=0:01:50.366  dur=0:00:00.366     T 가 뒤늦게 발화

fix:
  03:44:01:370  조각#2  rt=0:00:50.300
  03:45:00:478  All channels aligned                        initial alignment split 없음
  03:45:01:350  조각#3  rt=0:01:50.000  dur=0:00:59.700     자연 롤오버만 - 단일 절단
  03:46:01:368  조각#4  dur=0:01:00.000                     무회귀
```

조건 재현도: `smMax` 10309 vs 10360ms, 조각#2 시작 rt 양쪽 50.300, 자연 롤오버 절단도
양쪽 rt 110.000. 유일한 차이가 `logonly` 에만 있는 0.366초 조각이다.

`recording_time=2` 조건에서도 같은 대조를 했고, 거기서는 위상이 어긋나 증상(이중 절단)은
나오지 않았지만 원인(불필요한 emit)은 동일하게 관측됐다 — `logonly` 1건 / `fix` 0건,
절단 지점 169.966(앱 발화) vs 170.000(자연 롤오버).

#### 스냅백-롤오버 충돌 — drift 트리거에서는 운영 임계가 가드가 된다

자연 롤오버는 `duration*60` 이 분의 정수배이므로 조각이 열린 분내 초에 재발한다.
강제 분할은 `startSec` 직후 0~0.5초에 난다. 따라서 두 사건의 간격은 대략 drift 이고,
**drift 트리거**(`main.cpp:553`, `drift_ms >= split_max_msec`)로 난 강제 분할이라면
그 임계값 자체가 가드가 된다.

실측(위 `fix` 런): 조각 열림 03:44:01.370(분내 1370ms) -> 자연 롤오버 03:45:01.350
(분내 1350ms), 정렬 틱 03:45:00.478 -> 간격 872ms. 운영 기본 임계 5000ms 에서는 간격이
최소 4.5초로 GOP(0.5~1초)보다 훨씬 크다.

**단, 이 논증은 drift 트리거에만 적용된다.** `is_fully_aligned` 를 꺾는 트리거는 둘이고,
두 번째인 **skew 트리거**(`main.cpp:586`, `active_count > 1 && skew_ms >= split_diff_msec`,
기본 200ms)는 drift 와 독립이다. 전 채널 drift 가 임계보다 훨씬 작아도 채널 간 skew 만
200ms 를 넘으면 강제 분할이 나고, 그때 간격은 수백 ms — GOP 안쪽, 즉 충돌 조건이다.
유예(`SNAP_BACK_GRACE_PERIOD_MS`, 58000ms)도 목표 초 직후에 열린 조각에는 걸리지 않는다.
`main.cpp:572-578` 의 기존 주석도 wall-clock skew 가 내용이 정렬돼 있어도 불필요한
스냅백을 유발할 수 있다고 기록한다.

이 skew 경로는 **측정하지 않았다.** 다만 §3.31 의 이력 대조에서 2026-03-16~09-03 로그
195표본 중 running-time skew 가 193건 전부 0ms 였고 나머지 2건도 wall-clock 폴백
44/2ms 로 임계 200ms 미만이라, 그 기간에는 이 경로가 발동한 기록이 없다.

`-X 100`(정시성 임계 100ms, 테스트 전용)으로 낮추면 간격이 GOP 아래로 내려가 충돌이
열린다. 그 조건에서 수정 빌드로 재현했다:

```
03:31:00:279  Snap-back split (smMax:390ms)
03:31:00:279  forced split at running-time 290130916745   앱이 T=290.131 요청
03:31:00:407  Fragment opened rt=289.000                  자연 롤오버 (임계 289.966)
03:31:01:410  Fragment opened rt=290.133                  큐에 남은 T 가 발화 -> 같은 파일명
```

요청 T 290.130916745 와 뒤늦은 발화 290.133 의 차는 2.08ms 다. 이 경로는
`need_first_split` 수정으로 막히지 않으며, 이슈 #87 의 `split-at-running-time` 수정도
막지 못한다.

#### 파생 발견 — `split_sec != 0` (이슈 #99)

`drift` 는 분 경계(:00) 기준으로 재는데 목표는 `target_min*60 + startSec` 이다.
`-S 30` 으로 5분 실측(운영 기본 임계, `-X` 없음): `All channels aligned` **0건**,
`Snap-back split` **4건 = 전 주기**, 같은 파일명 중복 개방 **3건**. `smMax` 가
`-28730 ~ -28752ms` 로 고정되는 것이 서명이다 — 코드의 wrap 보정을 역산하면
조각은 분내 `:31.248 ~ :31.270` 에 열린다(목표 `:30` 직후). 정렬 자체는 정상인데
판정만 분 경계 기준이라 30초 어긋난 값을 본다. 현재 배포는 `-S` 를 쓰지 않아 영향이 없다.

#### 한계

- 2채널 구성이라 4채널 스트리밍 스레드 경합 조건과 다르다.
- 커널 모듈 `max9296.ko` 가 다른 세션의 수정본으로 교체된 상태였다
  (모듈 파일 md5 `d8b04e7ca5dea1a5d980b9d16a14c651`, 2026-09-08 02:29:29 부팅).
  세 갈래를 같은 드라이버 아래 연속 실행했으므로 대조의 내부 타당성에는 영향이 없다.
  (`modinfo -F srcversion` 은 `9E94D01E901C66E7416A46A` 를 냈으나 23자로 MD5 길이가
  아니다. 재확인 전에는 파일 md5 를 식별자로 쓴다.)
- 파일명 유일화는 배제했다 — `pim-package-jhw/ord/tcpServer.cpp:983-991` 이 요청 시각으로
  파일명을 정확히 재구성해 `cp` 하므로 접미사가 붙으면 ORD 이벤트 복사가 깨진다.

#### 원시 자료

아래 셋 모두 **저장소 밖**이다(`tmp/` 는 `.gitignore` 대상이라 clean checkout 에 없다).

- 타겟 `/root/i94/run94.{logonly1m,fix1m,fixX100,splitsec30}.log`
- `recording_time=2` 대조는 같은 디렉터리의 `run94.{logonly,base,fix}.log`
- 2026-09-04 런은 작성자 로컬 `tmp/target-192.168.214.4/record_skew_4ch_20260904/`

위 두 프로브(filesink truncate, splitmuxsink 같은 location)는 타겟이 필요 없고
이 절에 적힌 명령으로 어느 호스트에서나 재현된다 — 단 **구조적 결과만** 같다
(5파일 -> 1파일, 생존 파일이 마지막 조각과 같은 크기). 위 표의 바이트 총량은
`videotestsrc`/`jpegenc` 출력이 GStreamer 버전·빌드마다 달라 호스트마다 다르다.
filesink 프로브의 `1048576 -> 10 B` 는 정확히 재현되고 inode 값만 호스트 고유다.

## 4. 현재까지의 결론

2026-09-08 현재 다음 사항은 측정으로 확인됐다(1~39는 2026-08-13,
40~45는 2026-09-04 §3.31, 46~47은 2026-09-08 §3.32).
...
40. 1920×1080@15, 1280×720@30, 640×360@60(2회) 네 실행의 4채널 통제
    측정(LED 4채널 off, ISP 컴파일 기본값으로 4채널 동일)에서 분할 경계의
    running-time skew가 42표본 중 41건 0ms였다. 프레임 주기가 66.7ms에서
    16.7ms로 4배 좁아져도 이 결과는 바뀌지 않았다. 유일한 예외는 스냅백 강제
    분할 직후의 1건(16ms = 60fps의 1프레임)이며, 다음 정규 분할에서 회복됐다.
    상수 오프셋이 아니라 과도현상이다.
41. 같은 실행의 파일 프레임 수 대조에서 1920×1080@15는 네 채널 1800프레임,
    1280×720@30은 3600프레임, 640×360@60은 7200프레임으로 완전히 일치했고
    키프레임 수(각 60, 120, 240)도 같았다. 누적 네 개 분에서도 각각 7200,
    14400, 28800으로 동일했다.
42. ISI 인터럽트 기반 실측 프레임률이 네 실행 모두 기대값과 일치했다 —
    29.96/30, 59.20/60, 122.70/120, 118.60/120 /s. 기대값은 `fps × CSI수(2)`
    이며, ISI가 CSI당 결합 프레임 하나를 세기 때문이다. 640×360@60에서 자동
    노출로 인한 실제 프레임률 미달은 발생하지 않았다.
43. 과거 운영 로그(2026-03-16~09-03, 전 로테이션)의 `skew basis` 195건을 각
    표본의 구성과 조인해 전수 확인한 결과, running-time 기준 193건이 전부
    0ms였다. 0이 아닌 2건은 running-time을 얻지 못해 wall-clock으로 되돌아간
    회차이며 그 값은 벽시계 잡음이지 내용 오차가 아니다.
44. max9296 드라이버는 한 하드웨어 epoch 안에서 prepare fingerprint(해상도·
    fps·enable 조합) 변경을 `-ESTALE`로 거부한다. epoch은 실제 전원 전환 또는
    마지막 power user 해제에서만 오르므로, 구성을 바꿔 재기동하려면
    `cam_hard_reset.sh`가 필요하다. 시험 후 운영 구성으로 되돌아갈 때도 같은
    이유로 필요하다.
45. 스냅백 강제 분할은 채널 간 조각 경계를 한 프레임 어긋나게 만들 수 있다.
    640×360@60에서 스냅백 2회 중 1회에서 한 채널(ch1)만 한 프레임 이른 지점에서
    잘렸다. 강제 키프레임은 네 채널 모두에 전송된 상태였다. 어긋남은 다음 정규
    분할에서 사라졌다.

46. 같은 분에 분할이 두 번 일어나면 두 조각이 같은 파일명을 쓰고 `filesink` 가
    같은 inode 를 in-place truncate 하여 앞 조각이 사라진다(1,048,576 -> 10 B 로 실증).
    원인은 `need_first_split` 이 강제 분할 경로에서 소진되지 않아 이후 첫 '정렬됨'
    틱에서 불필요한 분할을 한 번 더 내는 것이었다. 로그 2줄만 넣은 중간 빌드로
    되돌림 대조를 했고, master 동작 1건 / 수정본 0건으로 갈렸다 (§3.32, 이슈 #94).
47. 스냅백 강제 분할이 자연 롤오버와 겹치는 충돌은 **drift 트리거**(`main.cpp:553`)로
    난 스냅백에서는 운영 기본 임계(`split_max_msec=5000`)가 가드가 된다. 자연 롤오버가
    조각이 열린 분내 초에 재발하므로 두 사건의 간격이 대략 drift 이고, 그 경로의 강제
    분할 조건이 `drift >= 임계` 이기 때문이다. `-X 100` 으로 임계를 낮추면 열린다.
    **skew 트리거**(`main.cpp:586`, 기본 200ms)는 drift 와 독립이라 이 논증이 적용되지
    않으며, 그 경로는 측정하지 않았다 — 다만 195표본 이력에서 발동 기록은 없다 (§3.32).

아직 증명되지 않은 사항은 다음과 같다.

1. 네 센서 노출 시작 위상이 실제로 sub-ms 단위에서 같은지 여부. §3.31이
   인코더 출력 이후 분할 경계까지를 1ms 미만으로 좁혔으므로 남은 미증명
   구간은 센서 노출 시각 자체다. 타임스탬프 기반 계측은 원리적으로 이를 볼
   수 없다 — 네 센서가 다른 순간을 노출해도 이후 체인의 PTS는 동일하게
   찍히기 때문이다. 광학 자극이 필요하다.
...
7. 15fps SEI OFF/ON A/B는 완료했다. 4채널 30fps(§3.31 1280×720@30)와
   4채널 60fps(§3.31 640×360@60)도 각각 15분 확인했다. 남은 것은 수시간
   memory plateau와 FHD 고부하 장시간 stress다.
