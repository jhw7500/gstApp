...
요청 T 290.1309 와 뒤늦은 발화 290.133 이 2ms 안에서 일치한다. 이 경로는
...
`-28730 ~ -28752ms` 로 고정되는 것이 서명이다 — 조각은 목표 `:30` 에 정확히 열리는데
판정만 30초 어긋난 값을 본다. 현재 배포는 `-S` 를 쓰지 않아 영향이 없다.
...
2026-09-04 현재 다음 사항은 측정으로 확인됐다(1~39는 2026-08-13,
40~44는 2026-09-04 §3.31).
