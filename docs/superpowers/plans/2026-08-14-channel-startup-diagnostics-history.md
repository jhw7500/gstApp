...
```bash
rtk grep -nE '동작은 정상이고 로그가 문제|제안[:]|추가[[:space:]]검토|커밋되지[[:space:]]않은[[:space:]]변경[[:space:]]5개|dist/pim/opt/pim/bin/gst[A]pp|/home/[^/]+/' docs/gstapp-handoff-2026-08-11-channel-logging.md
```
...
# PIM_PACKAGE_REPO를 지정하면 그 경로를 쓰고, 없으면 현재 Git common-dir의 sibling을 쓴다.
rtk sh -c '
pim_repo="${PIM_PACKAGE_REPO:-$(cd "$(git rev-parse --path-format=absolute --git-common-dir)/../.." && pwd)/pim-package-jhw}"
git -C "$pim_repo" rev-parse --is-inside-work-tree >/dev/null || exit 1
for sha in 7b2a112 7802a08 6e28828 63d8484; do
  git -C "$pim_repo" show --stat --oneline "$sha" || exit 1
done
'
...
4. 보존된 발췌에 `GST_MESSAGE_ERROR` handler 실행을 나타내는 로그가 관찰되지 않았음을 기록하되, 그 부재만으로 handler 미실행이나 하드웨어·드라이버 상태를 결론내리지 않는 한계를 쓴다.
...
```bash
rtk sh -c 'test ! -e docs/gstapp-handoff-2026-08-11-channel-logging.md'
...
! rtk grep -nE '제안[:]|추가[[:space:]]검토|커밋되지[[:space:]]않은[[:space:]]변경[[:space:]]5개|dist/pim/opt/pim/bin/gst[A]pp|/home/[^/]+/' docs/CHANNEL_STARTUP_DIAGNOSTICS_HISTORY.md
! rtk grep -nEi '(passw[o]rd|passw[d]|tok[e]n|sec[r]et|a[p]i[_-]?key)[[:space:]]*[:=][[:space:]]*[^[:space:]`"]+' docs/CHANNEL_STARTUP_DIAGNOSTICS_HISTORY.md
