...
#
# 적용 범위
#   이 스크립트는 max9296 PR #66(merge 7ec2949) 이전 v2.12에서 수행한 원인 판별 실험을
#   재현·보존한다. PR #66은 dual 시드의 게이트·값·주소를 쌍 단위로 만들었고 후속 #67은
#   채널별 노출 캐시와 하드웨어의 불일치를 해소했다.
#   수정된 드라이버는 exp_time_ch1 요청을 쌍 값으로 리다이렉트하거나 화해시킬 수 있다.
#   그 환경에서 X/Y가 정상인 것은 쌍 노출 불변식이 작동한다는 회귀 증거이지, 아래의 수정 전
#   인과 실험을 기각하는 증거가 아니다. 비교 전에 사용한 드라이버 버전·커밋을 반드시 기록한다.
#
...
#   수정 전 드라이버는 채널별 쓰기를 지원했다 — V4L2_CID_EXPOSURE_CH0/CH1(max9296.c:179-180),
#   적용부 :3283/:3346 이 각 채널 AP1302 에 직접 썼다.
#
...
#   PR #66 이후 X/Y가 정상이면 적용값이 실제로 갈렸는지 먼저 확인한다. 드라이버가 쌍 값을
#   유지했다면 "노출 발산 기각"이 아니라 수정된 불변식의 회귀 통과로 판정한다.
#
# 주의
