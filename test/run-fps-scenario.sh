...
for combo in $SUPPORTED; do
	...
	for r in $(seq 1 "$REPEAT"); do
      ...
      ' "$BACKUP" >"$OUT/.test.json" || {
		...
		CAM_STATE=$(systemctl is-active cam-operate.service 2>/dev/null); case $CAM_STATE in inactive|failed) ;; *) echo "!! cam-operate 상태가 [${CAM_STATE:-조회실패}] 다 - 시험 config 를 쓰지 않는다" >&2; exit 6;; esac
