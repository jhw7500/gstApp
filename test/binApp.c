...
static gpointer record_thread_run(gpointer user_data)
{
    // /dev/video3 장치로부터 영상 데이터를 spiltmuxsink를 사용하여 60초마다 날짜 형식으로 파일로 저장하는 파이프라인 설정
    GstElement *pipeline = gst_parse_launch("v4l2src device=/dev/video3 ! imxvideoconvert_g2d ! vpuenc_h264 ! h264parse ! splitmuxsink location=\"output-%d.mp4\" max-size-time=60000000000", NULL);
    if (!pipeline) {
        g_printerr("파이프라인을 생성할 수 없습니다.\n");
        return -1;
    }
...
static gboolean change_file_name(GstElement *filesink) {
    ...
    g_message("timeout state : %s", gst_element_state_get_name(state));
    gst_element_set_state(binLF, GST_STATE_PAUSED);
    //gst_element_set_state(filesink, GST_STATE_PAUSED);
    gst_element_get_state(filesink, &state, NULL, GST_CLOCK_TIME_NONE);
...
gboolean timeout(GstElement *bin)
{
  gboolean result = FALSE;
  GstState state;

  gst_element_get_state(bin, &state, NULL, GST_CLOCK_TIME_NONE);
  
  g_message("timeout state : %s", gst_element_state_get_name(state));
  if(state == GST_STATE_PAUSED)
  {
    gst_element_set_state(GST_BIN(bin), GST_STATE_PLAYING);
    g_message("to PLAYING");
  }
  else if(state == GST_STATE_PLAYING)
  {
    gst_element_set_state(GST_BIN(bin), GST_STATE_PAUSED);
    g_message("to PAUSED");
  }
  else
  {
...
int main(int argc, char *argv[]) 
{
    ...
    //gst_element_set_state (binLF, GST_STATE_NULL);
    gst_element_set_state (pipeline, GST_STATE_NULL);
    //gst_object_unref(binMain);
    //gst_object_unref(binLF);
    gst_object_unref(pipeline);
