...
static GstPadProbeReturn pad_probe_cb (GstPad * pad, GstPadProbeInfo * info, gpointer user_data) 
{
    ...
    printf("current_bin:%ld, current_file:%ld\n", current_bin, current_file);                                          

    {                                                                           
       char file_location[32];                                                  
       sprintf(file_location, "recording_%ld.mp3", current_file);
       printf("7\n");              
...
int main(int argc, char *argv[]) {                                              
    ...
    vorbisenc[0] = gst_element_factory_make("lamemp3enc", "vorbisenc0");         
    vorbisenc[1] = gst_element_factory_make("lamemp3enc", "vorbisenc1");         

    oggmux[0] = gst_element_factory_make("mp4mux", "oggmux0");                  
    oggmux[1] = gst_element_factory_make("mp4mux", "oggmux1");                  
